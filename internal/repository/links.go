...
func ListLink(pool *pgxpool.Pool, userId int) ([]models.ShortLink, error) {
	query := `
		...
		FROM short_links sl
		LEFT JOIN clicks c ON c.shortlinkid = sl.id
		WHERE sl.userid = $1
		GROUP BY sl.id
...
func UpdateLink(pool *pgxpool.Pool, userId int, slug string, originalUrl string, customSlug *string) (*models.ShortLink, error) {
	...
	_, err = pool.Exec(context.Background(),
		`UPDATE short_links 
		 SET originalurl=$1, shorturl=$2, updated_at=$3 
		 WHERE id=$4`,
...
func GetClickCount(pool *pgxpool.Pool, shortLinkID int) (int64, error) {
	var count int64
	query := `SELECT COUNT(*) FROM clicks WHERE shortlinkid = $1`

	err := pool.QueryRow(context.Background(), query, shortLinkID).Scan(&count)
	if err != nil {
		return 0, err
	}
